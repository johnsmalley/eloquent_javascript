...
// converts ANCESTRY_FILE string in to usable values
var ancestry = JSON.parse(ANCESTRY_FILE);

function centryDied(person) {
	return Math.ceil(person.died / 100);
}

function groupBy(array, grouping) {
	return array.reduce(function(group, element) {
    if (group[grouping(element)]) {
    	group[grouping(element)].push(element);
    } else {
...
var agesByCentry = groupBy(ancestry, centryDied);


for(var centry in agesByCentry) {
 
  var centryAverageAge = average(agesByCentry[centry].map(function(person) {
    return person.died - person.born;
  }));

  console.log(centry + ": " + Math.round(centryAverageAge * 10) / 10);
}
